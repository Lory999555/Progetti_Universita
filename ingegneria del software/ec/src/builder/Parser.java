...
public class Parser {
	...
	private void factb(){
		if(token==Token.NOT){
			...
			expr();
			if(token!=Token.EQ && token!=Token.NEQ && token!=Token.GE && token!=Token.GT && token!=Token.LE && token!=Token.LT){
				throw new SyntaxException("Atteso un operatore di confronto dopo una expr");
			}
			Condition c;
			if(token==Token.EQ) c=new EQ();
			else if(token==Token.NEQ) c=new NEQ();
			else if(token==Token.GT) c=new GT();
			else if(token==Token.GE) c=new GE();
			else if(token==Token.LT) c=new LT();
			else c=new LE();
			c.setLeft(root);
			token=lexer.next();
			expr();
	...
	private void expr(){
		if(token==Token.PLUS){
			...
			while(token==Token.PLUS || token==Token.MINUS ){
				Condition c;
				if(token==Token.PLUS) c=new Plus();
				else c=new Minus();
				c.setLeft(root);
				token=lexer.next();
				term();
	...
	private void term(){
		...
		while(token==Token.DIV || token==Token.MULT || token==Token.REM){
			Condition c;
			if(token==Token.DIV) c=new Div();
			else if(token==Token.MULT) c=new Mult();
			else c=new Rem();
			c.setLeft(root);
			token=lexer.next();
			termp();
	...
	private void fact(){
		...
		else throw new SyntaxException("Atteso una variabile, un valore o una parentesi quadra");
	}
